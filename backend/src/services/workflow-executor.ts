...
async function callAI(
    ...
    // Build multi-turn contents: system + all previous turns
    const contents = [
...
// Execute a single node, passing full conversation chain
async function executeNode(
    ...
    switch (node.type) {
        case 'ai_agent': {
            ...
            const systemPrompt = `你是「${botName}」。\n${botPrompt}\n\n你将收到一段对话历史（包含用户的输入和之前其他智能体的分析结果）。请在此基础上，发挥你的专业能力继续深入分析和回答。`;

            // Build conversation for this agent: include all prior context
            const history = [
        ...
        case 'condition': {
            ...
            if (keyword && inputText.includes(keyword)) {
                return `[条件成立] ${inputText}`;
            }
            return `[条件不成立] ${inputText}`;
        }
...
export async function executeWorkflow(
    ...
    try {
        const canvas: CanvasData = JSON.parse(workflow.canvasData);
        ...
        // Conversation chain: accumulates all agent interactions for context passing
        const conversationChain: Array<{ role: string; text: string; agentName: string }> = [];

        // Get initial input from execution record
        ...
        for (const nodeId of order) {
            ...
            // Determine input: from connected parent nodes or last output
            ...
            try {
                ...
                // Add to conversation chain so next agent has full context
                if (node.type === 'input') {
                    ...
                    const agentName = (node.data.botName as string) || (node.data.label as string) || 'AI';
                    conversationChain.push({
                        role: 'assistant',
                        text: output,
                        agentName,
        ...
        await prisma.workflowExecution.update({
            ...
            data: {
                status: 'completed',
                output: lastOutput.slice(0, 10000),
                stepLogs: JSON.stringify(stepLogs),
                finishedAt: new Date(),
            },
        });
