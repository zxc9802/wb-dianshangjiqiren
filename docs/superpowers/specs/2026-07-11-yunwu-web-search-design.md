...
将主站联网搜索模型切换为 Yunwu 的 `deepseek-v3-search`，同时保持前端现有的“关闭 / 自动 / 开启”联网模式和当前回答模型选择逻辑不变。
...
先将现有测试改为期望 `deepseek-v3-search` 请求格式，并确认它在旧实现下失败；再实施最小代码修改使测试通过。最后运行联网搜索专项测试、相关聊天测试、类型检查和生产构建，确认没有破坏现有聊天流程。
