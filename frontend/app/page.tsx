    ...
    <div className={styles.layout}>
      <header className={styles.header}>
        ...
        <div className={styles.headerRight}>
          ...
          {isAuthenticated ? (
            <>
              <button onClick={() => router.push('/profile')} className={styles.avatarBtn}>{user?.account?.slice(0, 1) || '我'}</button>
            </>
          ) : (
            ...
            <div className={styles.heroGreeting}>
              <h2 className={styles.heroGreetingTitle}>
                {isAuthenticated && user?.account
                  ? <><span className={styles.heroGreetingHighlight}>{user.account}，你好</span><br />需要我为你做些什么？</>
