...
type Group = {
    userId: string; nickname: string; email: string; groupName: string; source: string; model: string;
...
const sourceLabel = (source: string) => source === 'main' ? '主站' : source.replace('sso:', 'SSO · ');
    ...
    return <main className={styles.page}>
        ...
        {data && <>
            ...
            <div className={styles.table}><table><thead><tr>{['员工 / 组别', '来源 / 模型', '调用 / 异常 / 待完成', '输入 Token', '输出 Token', '缓存命中', '总 Token', '金额'].map(s => <th key={s}>{s}</th>)}</tr></thead><tbody>
                {groups.map((g, i) => <tr key={i}><td>{g.nickname || g.email || g.userId}<small>{g.groupName || '未分组'}</small></td><td>{sourceLabel(g.source)}<small>{g.model}</small></td><td>{g.calls} / {g.failed} / {g.pending}</td><td>{number(g.inputTokens)}</td><td>{number(g.outputTokens)}</td><td>{number(g.cachedInputTokens)}</td><td>{number(g.totalTokens)}<small>{g.tokenBasis === 'estimated' ? '估算 Token' : g.tokenBasis === 'missing' ? '未返回' : '接口报告'}</small></td><td>{money(g.amount, g.currency)}<small>{basis(g.costBasis)}</small></td></tr>)}
            ...
            <section><h2>模型估算费率</h2><p>按实际供应商和令牌分组填写。每百万 Token 单价；图片按次计价可填写“每次”。这里的费率不会改变员工积分余额，也不等于供应商账单。</p>
                {!editingRates ? <button onClick={() => { setRates(data.rates); setEditingRates(true); }}>配置费率（{data.rates.length} 个模型）</button> : <>
                    <div className={styles.table}><table><thead><tr>{['供应商域名', '模型', '币种', '输入 / 百万', '输出 / 百万', '缓存命中 / 百万', '缓存写入 / 百万', '每次（可空）', '操作'].map(v => <th key={v}>{v}</th>)}</tr></thead><tbody>{rates.map((rate, i) => <tr key={i}>
