...
import { normalizeUsageProvider } from '@/app/lib/usage-pricing';
import { pricedUsageCte } from '@/app/lib/usage-report-pricing';
...
export async function GET(req: NextRequest) {
    try {
        ...
        const pricedEvents = pricedUsageCte(filters, rates);
        const [groups, rows, counts, users, sources] = await Promise.all([
            prisma.$queryRaw(Prisma.sql`${pricedEvents} SELECT e.user_id AS "userId", u.nickname, u.email, u.group_name AS "groupName",
                ...
                SUM((e.data->>'cacheWriteTokens')::numeric)::float8 AS "cacheWriteTokens",
                SUM((e.data->>'totalTokens')::numeric)::float8 AS "totalTokens",
                SUM((e.data->>'amount')::numeric)::float8 AS amount
                FROM priced_events e LEFT JOIN users u ON u.id = e.user_id
            ...
            prisma.$queryRaw(Prisma.sql`${pricedEvents} SELECT e.id, e.created_at AS "createdAt", e.data, u.nickname, u.email
                FROM priced_events e LEFT JOIN users u ON u.id=e.user_id
...
const rateSchema = z.array(z.object({
    provider: z.string().trim().min(1).max(100).transform(normalizeUsageProvider), model: z.string().trim().min(1).max(200),
    currency: z.enum(['USD', 'CNY']), inputPerMillion: rateNumber.nullable(), outputPerMillion: rateNumber.nullable(),
    cachedPerMillion: rateNumber.nullable(), cacheWritePerMillion: rateNumber.nullable(), perCall: rateNumber.optional(),
    imageInputPerMillion: rateNumber.nullable().optional(),
